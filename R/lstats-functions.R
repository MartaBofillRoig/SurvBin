...
#' @param w1 A scalar parameter that controls the type of test (see Weights).
#' @param w2 A scalar parameter that controls the type of test (see Weights).
#'
#' @export
#'
...
lstats <- function(time, status, binary, treat, tau0=0, tau=NULL, taub=NULL, rho=0, gam=0, eta=1, w1=0.5, w2=0.5){
  ...
  sigma_sb <- survbinCov_kernel(db$time,db$status,db$binary,db$treat, tau0, tau, taub, rho, gam, eta)

  # Calculate the statistic
  u_bs = w1*test_b + w2*test_s

  # Calculate the estimated variance
  variance =  w1^2 + w2^2 + 2*w1*w2*sigma_sb/(sigma_b*sigma_s)
